...
The current bootstrap branch has no tracked `evidence/` tree. Restore it only
when it actually exists; never pass an absent path to `git restore`:

```bash
if git cat-file -e "$BOOTSTRAP_BRANCH:evidence" 2>/dev/null; then
  git restore --source="$BOOTSTRAP_BRANCH" -- evidence
fi
```
