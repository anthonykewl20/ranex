...
6. the Engineering Reference Application Map's governed application of
   ...
   *System Design Interview*, *The Clean Coder*, *Designing Data-Intensive
   Applications*, *Fundamentals of Software Architecture*, *Clean
   Architecture*, and *A Philosophy of Software Design*.

The ten works are practice inputs, not decorative citations. At contract
readiness, `engineering-practices.json` registers each adopted practice with
...
- evaluates all ten source families for applicability;
...
- The ten foundational works affect actual task behavior and verification
...
- Add `architecture/contracts/engineering-practices.json` to the target
  contract registry.
...
4. The licensing manifest no longer classifies the deleted file as a live
   repository file.
5. The machine-contract target includes `engineering-practices.json`.
...
7. All 18 full-text PDF/Markdown representations remain ignored/local-only and
   cannot enter a public package or release.
...
on 2026-07-28 that every manifested foundational work be practiced and used in
developing Ranex and that the legacy root implementation guide be removed so
it cannot pollute the build. The live recursive inventory establishes ten
works/18 representations. This ADR records that decision; it does not claim a
