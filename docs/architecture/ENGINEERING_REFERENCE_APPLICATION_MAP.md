...
| Corpus count/size | 17 files / 86,856,268 bytes; nine conceptual works, eight PDF/Markdown pairs, and one PDF-only work |
| Manifest SHA-256 | `24517b0975ed5c4c601ac482f6403c28b3570719333c1f1f1c83bff7c59a8e18` |
| Practice-source dataset | [Nine-family engineering-reference practice registry](../research/engineering-reference-practice-registry.json) |
...
The current reference set has nine conceptual works represented by 17 stored
artifacts. Eight works have a PDF and a lossy Markdown extraction; *Clean
Architecture* is PDF-only. Representations of one intellectual work share one
source-family ID and are never independent corroboration.
...
All nine references are **major** because each owns a named part of the
...
- all seventeen full-text artifacts are `LOCAL_ONLY`;
...
| `DISCOVER` | Pragmatic requirements exploration/reversibility; System Design clarification/scale prompts; DDIA fault/load vocabulary; Fundamentals characteristic discovery | Current behavior, user/actor evidence, assumptions, unknowns, falsifiable hypothesis, quality priorities, and scale envelope |
...
| `DESIGN` | All nine families: discipline coverage, complexity, boundaries, dependency direction, data/failure semantics, topology trade-offs, alternatives, ADRs, and fitness functions | Full owner/boundary/API/state/data/effect/file/failure/recovery map plus characteristics, alternatives, ADR, and falsifiers |
...
| `BUILD` | Code Complete construction design; Clean Code and Clean Architecture boundaries/testability; Pragmatic contracts/generation; DDIA exact state/effect semantics | Dependency-conforming code, tests, docs, generated contracts, immutable run evidence, and no hidden scope |
| `VERIFY` | SWEBOK testing/quality; Clean Coder layered testing/acceptance; Pragmatic regression learning; boundary/encapsulation fitness; DDIA crash/replay/stale-worker/duplicate tests | Exact-subject independent evidence, deterministic gates, failure-path proof, architecture fitness results, and no self-approval |
...
| `MAINTAIN_OR_RETIRE` | SWEBOK maintenance/configuration; Code Complete maintainability; Pragmatic reversibility; DDIA history/retention/erasure tension | Controlled change or removal, consumer/data/access migration, compatibility evidence, and governed retained/erased history |
| `IMPROVE` | SWEBOK process/measurement; Clean Coder learning/mentoring; Pragmatic continuous learning; Fundamentals evolutionary fitness functions | Evidence-bound capability assessment, bounded experiment, guardrails, corrective action, and human decision |
...
now freezes nine source families and 34 stable practice-source IDs. Its status
...
consumes that dataset into `architecture/contracts/engineering-practices.yaml`
...
The compiler evaluates all nine source families when producing the accepted
...
It does not ask a model to reread or summarize all nine works for every task.
...
12. a changed reference does not trigger impact review of the rules it
    supports;
13. a task bypasses the compiled nine-family registry, or its compact
...
**nine-family practice-source attachment set** for the exact manifested
...
> **Nine works and seventeen representations frozen and source-mapped; rights,
