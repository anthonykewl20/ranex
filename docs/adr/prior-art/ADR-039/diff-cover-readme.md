...
coverage reports as arguments. You may specify any arbitrary number of
coverage reports:
...
over the generated java bytecode and should be integrated in the build
framework.
...
The parser will only react to configuration files ending in `.toml`.
...
**Solution**: ``diff-quality`` assumes you have the tool you wish to run against the diff installed.
...
``--violations`` option of ``diff-quality``, e.g.:
...
.. code:: python
    ...
    class SQLFlakeViolationReporter(BaseViolationReporter):
        supported_extensions = ['sql']

        def __init__(self):
            super(SQLFlakeViolationReporter, self).__init__('sqlfluff')

        def violations(self, src_path):
            return [
                Violation(violation.line_number, violation.description)
                for violation in get_linter().get_violations()
            ]
    ...
    def diff_cover_report_quality():
        return SQLFlakeViolationReporter()
...
* ``diff-quality`` is looking for a plugin function:
  ...
  * Named ``diff_cover_report_quality``.  (This distinguishes it from any other
    plugin types ``diff-cover`` may support.)
