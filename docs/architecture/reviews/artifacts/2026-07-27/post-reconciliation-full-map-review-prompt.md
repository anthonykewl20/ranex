# Ranex Post-Reconciliation Full-Map Architecture Review
...
- `post-reconciliation-source-manifest.sha256` inventories the complete frozen
  documentation/research/legal subject. It is the authoritative subject list.
- `post-reconciliation-review-bundle-manifest.sha256` identifies the files
  whose text is attached to the request. Raw Kimi stages, book full texts,
  binary/rendered projections, historical research reports not needed for the
  current contract audit, and prior raw model outputs are intentionally not
  placed in the model context; audit their treatment through the exact
  manifests, current normative synthesis, and reconciliation records. Do not
  claim to have directly read an unattached artifact.
