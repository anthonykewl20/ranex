...
for the 29 fixed architecture decisions. A contract compiler must parse it,
...
decisions:
  ...
  - decision_id: "DEC-RANEX-017"
    ...
    selected: "single worker default; fenced measured fan-out; isolated writes; serialized human landing"
    owner: "process-owner"
    governing_adr: "ADR-0005"
    alternatives: ["unbounded fan-out", "always-parallel writers"]
    fitness_functions: ["FF-FLEET-001", "FF-EXP-001"]
    status: "ACCEPTED"
  ...
  - decision_id: "DEC-RANEX-025"
    name: "providers"
    selected: "BYOK or local qualified release-pinned routes only"
    owner: "routing"
    governing_adr: "ADR-0006"
    alternatives: ["first-party paid account", "unqualified dynamic provider"]
    fitness_functions: ["FF-ROUTE-001", "FF-SUPPLY-001"]
    status: "ACCEPTED"
  - decision_id: "DEC-RANEX-026"
    name: "Nous-commercial-product"
    selected: "removed: no billing, credits, subscriptions, managed tool pool, purchase, or fallback"
    owner: "product-owner"
    governing_adr: "ADR-0006"
    alternatives: ["hide commercial UI", "retain dormant commercial runtime"]
    fitness_functions: ["FF-DECOMM-001"]
    status: "ACCEPTED"
  - decision_id: "DEC-RANEX-027"
    name: "remote-model-catalog"
    selected: "cannot activate models; qualified catalog is release-pinned"
    owner: "routing"
    governing_adr: "ADR-0005"
    alternatives: ["remote auto-activation", "model-selected route"]
    fitness_functions: ["FF-ROUTE-001", "FF-SAFE-001"]
    status: "ACCEPTED"
...
Fitness IDs defined in ADR-0003 through ADR-0005 resolve there. The remaining
registry-level functions resolve as follows:
...
| `FF-PATH-001` | Canonical source-path registry has one context root and generated/source layout matches it. |
...
| `FF-DECOMM-001` | Static and runtime scans prove zero billing/credits/subscription/purchase/managed-service path. |
...
A change to one row requires a new accepted ADR naming the old decision ID,
...
to the superseding ADR. IDs are never recycled or silently renumbered.
