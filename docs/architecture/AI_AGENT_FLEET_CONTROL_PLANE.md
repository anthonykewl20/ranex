...
authenticated human decisions and accepted ADRs
        ...
        agent_collaboration assignment + fenced lease
           ...
           governed_execution Run + governor
             ...
             |                       |
             v                       v
      isolated worker             CapabilityBus
      proposal/evidence      policy eligibility
...
| Assignment, offer, claim, lease, heartbeat, mailbox, and handoff | `agent_collaboration` | No gate, permit, effect, merge, or release authority |
...
| Hierarchical budget, reservation, quota, and attributed usage | `resource_governance` | Child work is charged transitively to every parent reservation |
...
| Model/provider selection inside an accepted route policy | `routing` | A friendly model alias is never identity proof |
...
session, route lock, workspace, capability profile, issued time, expiry,
heartbeat policy, monotonically increasing fencing epoch, and aggregate
version.
...
5. the required role, route, harness, sandbox, and capabilities are qualified;
6. a valid workspace and allowed-path plan exist for writing work;
7. resource reservations exist for the full parent/child chain; and
8. maker/checker separation remains satisfiable.
...
- child-assignment count and maximum nesting depth;
- concurrent-process and concurrent-worker ceilings;
- retry, provider-fallback, and consecutive-failure limits;
- artifact/log growth and context-compilation budgets; and
- human-response expiry.

The caller owns one absolute budget tree. A child deadline or budget cannot
outlive or exceed its parent.
...
| `STAR_READ` | One coordinator, independent read-only workers | Coordinator is not a decision authority; artifacts are immutable |
...
| `STAGED_CHAIN` | Planner proposal → implementer → verifier | Each stage has a new packet/attempt; required independence enforced |
...
Worker-to-worker communication uses durable envelopes and artifact references,
not unrecorded shared memory.
...
The next worker pulls referenced artifacts directly. An orchestrator's summary
is a navigation aid, not a substitute for the source.
...
Every child charge is atomically attributed to its leaf and ancestors. Retry,
fallback, subagent spawn, review, polling, and generated output consume the same
parent budget unless a human-authorized new reservation is issued.
...
- topology, worker count, model/route/harness, prompts, tools, budgets, and
  time limits;
...
actual provider/model/transport, qualification, sandbox, tool permissions,
context/output limit, price observation, and fallback chain.
...
- budget laundering through child workers or provider fallback;
...
- model/tool/provider latency, errors, fallback, and drift; and
...
| Child budget escape | Reservation-tree mismatch | Deny and incident/corrective action | Charge no parent |
...
3. A child spawns nested work and attempts to evade budget; every charge reaches
   all ancestors and the hard cap stops the tree.
...
7. A planner proposes overlapping writers and broader capabilities; the
   deterministic scheduler rejects the plan.
...
11. Provider/model/permission facts drift behind a stable alias; the route and
    dependent assignments enter probation/invalidation.
...
ADR-0005 selects one worker, explicit read-only fan-out, isolated write
worktrees, serialized human landing, static qualified routes, the release-
pinned lease profile above, a local process behind the worker-runtime port, and
inactive learned routing/orchestration. Poll/push dispatch and reviewer route
are deterministic release-profile fields selected from registered task/risk
facts, not worker discretion.
...
or human-review requirement. No selected default is claimed implemented until
