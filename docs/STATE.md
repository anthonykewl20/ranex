...
oracle-science §5. `governance/calibration/base-freeze-v1.json` (new
committed gauge class, ADR-063) binds kernel identity (kernel_commit
...
improvement claim lacking a committed freeze citation, paired marginals
on the freeze's named axes (baseline = the freeze's own number),
reconciled arithmetic, a tau derived from the freeze (L3's rejected 0.80
is a refused cause), or an evidence digest; the freeze is read committed
at the ref (seam C: uncommitted gauge edits are refused). Receipt:
...
decision bytes identical ×3). Kernel untouched: verdict.py unmoved.
...
Queue: #112 (the C2 ladder over SLICE-092's rung pointers), #114, #115,
milestone 7, then #102, #105, #106, #119. P2 (C3) and P3 (C4) ship in
parallel per DIRECT 009.

Suite status, measured 2026-09-25 on the merged tree (uv run --frozen
pytest -q, clean tree): 2170 passed / 91 skipped, 9 failed / 9 errors —
the standing host-glibc / reproducible-build drift family (slice036
selectors, approved-batch contract, specification-batch qualification,
gating stage_08b/slice009, suite-freeze goldens; same set as the pristine
base, audits/2026-09-23-live-observer/base-failures.log) PLUS two
slice004 sigkill-drain rows failing identically at plain origin/main
b7f039ba0 (reproduced there this session — inherited, not this branch).
The owner re-pin main already records is still needed. Suite manifest refrozen to 2279 IDs on the merged tree
(2217 base + #111's 21 + SLICE-093's 41), load_manifest verified.
...
hosting stay UNVERIFIED/UNIMPLEMENTED as detailed in MAP.
