...
| Exact corpus | Ten conceptual works represented by eight PDF/Markdown pairs and two PDF-only works |
| Corpus count/size | 18 files / 88,515,353 bytes |
...
| Work/representation index SHA-256 | `54316a04405e47d96adbc9f80a4c2d30294ba886dd1f7923c74f0870e04d5b41` |
...
The live directory now contains **18 stored artifacts representing ten
...
The four works added after the six-work review are:
...
2. *Fundamentals of Software Architecture*, a First Edition PDF/Markdown pair;
   and
3. *Clean Architecture*, a PDF-only Calibre representation; and
4. *A Philosophy of Software Design*, a PDF-only First Edition v1.01
   representation.
...
component boundaries, enforceable encapsulation, change amplification,
cognitive load, information leakage, module depth, and comparative design.
They also expose
...
| `ENGREF-APOSD-1E` | A Philosophy of Software Design, First Edition v1.01 | PDF `48cee0a4a0dfeeea6617b71252bf989df8a616ca389b9fc9e1bb5b1627441d61`; no Markdown representation |

The authoritative path, byte-size, representation, and grouping details are in
the machine-readable index. The manifest contains all 18 paths and no
...
| `ENGREF-APOSD-1E` | First Edition v1.01, November 2018; local PDF metadata and front matter identify John K. Ousterhout | PDF-only, 188 sequence pages; qualitative Java/C++ module-design examples; no acquisition/custody record | Copyright 2018 John K. Ousterhout; all rights reserved; reproduction requires written permission |
...
### 5.10 `ENGREF-APOSD-1E`

The PDF has no Markdown derivative. Locators below are sequence pages in the
exact 188-page First Edition v1.01 PDF:

| Claim family | Exact PDF sequence locator | Ranex disposition |
|---|---|---|
| Complexity symptoms: change amplification, cognitive load, and unknown unknowns | Chapter 2, pages 18–24 | Adopt as separate qualitative diagnostics backed by change and unfamiliar-reader evidence, never as additive points |
| Deep modules and shallow-interface risk | Chapter 4, pages 31–39 | Adapt: justify consumer-facing surface by useful complexity hidden; reject shallow indirection without creating god modules |
| Information hiding and leakage | Chapter 5, pages 40–49 | Adopt one semantic owner and a narrow complete contract; keep authority, denial, security, failure, recovery, and operations facts explicit |
| Design it twice | Chapter 11, pages 99–101 | Adapt proportionately: significant decisions compare status quo and at least two viable shapes, including interface burden, consequences, reversibility, and falsifiers |

The source is an opinionated, experience-based design reference. Its
Java/C++ examples and qualitative notions of module depth do not establish a
universal package tree, class count, scalar architecture score, or runtime
conformance. Ranex therefore binds the practices to exact coupling,
boundary-fit, public-contract, decision, and independent-review evidence.

## 6. Lifecycle and architecture use map

| Core stage | Added ten-family contribution |
|---|---|
...
| `DISCOVER` | System Design scale questions; DDIA load/fault vocabulary; Pragmatic reversibility; Fundamentals characteristic discovery; APOSD comparative design for consequential choices |
| `SPECIFY` | SWEBOK requirements; Pragmatic contracts; Clean Coder acceptance examples; DDIA reliability/performance scenarios |
| `DESIGN` | All ten families: discipline coverage, complexity symptoms, deep modules, information hiding, boundaries, dependency direction, data/failure semantics, topology tradeoffs, alternatives, and ADRs |
| `PLAN` | SWEBOK management/economics; Clean Coder estimate/commitment; Pragmatic reversible routes; Fundamentals tradeoffs |
| `BUILD` | Code Complete construction design; Clean Code, Clean Architecture, and APOSD boundaries/information hiding; Pragmatic generation; DDIA exact state/effect semantics |
| `VERIFY` | Risk-layered falsification; architecture fitness functions; dependency/encapsulation and information-leakage tests; change/comprehension challenge; crash, replay, stale-worker, duplicate, and reconciliation tests |
...
| `MAINTAIN_OR_RETIRE` | SWEBOK maintenance/configuration; Code Complete maintainability; Pragmatic reversibility; APOSD change amplification; event/history retention and erasure |
| `IMPROVE` | SWEBOK process/measurement; Clean Coder learning; Fundamentals evolutionary fitness functions; APOSD complexity and alternative review; evidence-bound experiments |

This is a source-attachment map. A task-specific profile still evaluates all
ten families as `APPLICABLE`, `NOT_APPLICABLE`, or `UNKNOWN`.
...
| APOSD deep modules versus bounded ownership and god-module risk | Seek a simple interface relative to useful hidden complexity, while bounding responsibility, authority, fan-in/fan-out, failure behavior, and change coupling |
...
A pattern name, quotation, ten-family citation count, or model consensus is
...
- all ten stable source-family IDs;
- 38 stable practice IDs;
...
Every one of the 18 full-text paths is classified in
...
1. live discovery produces a book path absent from the 18-entry manifest;
2. `sha256sum -c` fails for any entry;
3. the index does not contain ten unique families, 18 unique paths, and
   88,515,353 representation bytes;
...
11. a ten-family mapping is reported as implementation or conformance; or
...
> **Ten works / eighteen representations frozen and source-reconciled;
