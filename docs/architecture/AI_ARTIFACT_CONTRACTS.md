...
| Owner decisions | [ADR-0001](./decisions/ADR-0001-established-sdlc-governs-ai-work.md); [ADR-0002](./decisions/ADR-0002-retire-legacy-implementation-guide.md) |
...
from the exact `engineering-practices.yaml` registry and
...
- evaluates all nine registered source families as `APPLICABLE`,
...
- all 18 ADR-0007 topology rules, all 19 ADR-0008 TDD rules, all ten ADR-0009
  boundary/feedback rules, all 18 allowed test roots, all 232 path contracts,
...
- each of the 47 topology/TDD/boundary-fitness rules has one exact
...
- all nine stable source-family IDs and 34 stable practice-source IDs are
...
- every runtime producer to be the registered context and unable to forge
  another producer's record;
- hidden-fixture isolation and review independence through a real harness;
- exact-revision compatibility/upcaster fixtures for any breaking change; and
- an enacted `src/ranex` tree plus representative test roots before source
  topology, import-cycle, production/test-path parity, or TDD conformance can
  move from `NOT_ASSESSED`; and
- authenticated human acceptance of the resulting `AI-G2` evidence.
