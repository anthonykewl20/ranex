...
**Slice:** `n/a — precedes future measurement slices; SLICE-014 remains next`
...
## More Information
This proposed ADR corrects MAP §11.3: the frozen target fixes the outcome instrument, not the treatment. Status transitions are `proposed` → `accepted` or `rejected`/superseded by owner decision; acceptance may precede P0, and evidence may later drop or supersede it. `docs/STATE.md` keeps the durability track next; a flywheel prototype pointer is recorded only when P0 opens. F4 has no scheduled unblock path while RISK-06/07/19 remain open. The six vendored files and `NOTICE.md` are internal evidence of fetched source bytes; their hashes do not prove URL provenance without an independent fetch.
