...
def _worktree_dirty_paths(worktree: Path, candidate_paths: Sequence[str]) -> tuple[str, ...]:
    ...
    Porcelain v1 with ``-z`` keeps arbitrary path names unambiguous, and
    ``--untracked-files=all`` prevents Git from collapsing wholly-untracked
    directories into one path.  For a rename or copy, its first pathname is the
    post-image and the second NUL field is the pre-image; both can hide an
    overwrite and are therefore dirty paths.

    Ignored files do not appear in status, yet a candidate can replace one at a
    changed path during fast-forward synchronization.  Query only candidate
    paths for ignored, non-index files rather than enumerate the worktree's
    entire ignored universe.  Skipped-worktree files remain a residual: Git
    reports them clean even when their on-disk contents differ.
    """
    ...
    while index < len(records):
        ...
        index += 1
    if candidate_paths:
        # A fast-forward can replace an ignored file at a candidate path, but
        # status omits ignored files. Limit this query to the candidate's paths.
        ignored = git(
            worktree,
            "ls-files",
            "--others",
        ...
        if ignored.returncode != 0:
            detail = ignored.stderr.strip() or "git ls-files failed"
            raise ValueError(f"cannot read ignored paths at {worktree}: {detail}")
        paths.update(path for path in ignored.stdout.split("\0") if path)
    return tuple(sorted(paths))


def _worktree_sync_repair(
    worktree: Path,
    start_commit: str,
    candidate: str,
    target_ref: str,
) -> str:
    """Return the shell-quoted commands to fast-forward a stale worktree."""

    return (
        f"git -C {shlex.quote(str(worktree))} "
        f"checkout --detach {start_commit} && "
        f"git -C {shlex.quote(str(worktree))} "
        f"merge --ff-only {candidate} && "
        f"git -C {shlex.quote(str(worktree))} "
        f"symbolic-ref HEAD {shlex.quote(target_ref)}"
    )
...
def _recover_task_merges(
    ...
    for intent in pending:
        ...
        if landed:
            try:
                checked_out_worktree = _checked_out_target_worktree(repository_root, target_ref)
                if checked_out_worktree is not None:
                    worktree_head = git(checked_out_worktree, "rev-parse", "HEAD")
                    if worktree_head.returncode == 0:
                        observed_worktree_head = worktree_head.stdout.strip()
                        if observed_worktree_head != candidate:
...
def cmd_task_merge(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        if checked_out_worktree is not None:
            changed = git(repository_root, "diff", "--name-only", "-z", observed_tip, candidate)
            ...
            changed_paths = tuple(path for path in changed.stdout.split("\0") if path)
            dirty_paths = _worktree_dirty_paths(checked_out_worktree, changed_paths)
            conflicts = set(dirty_paths).intersection(changed_paths)
            if conflicts:
                return _merge_refuse(
                    journal,
                    intent,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    host_build = host.add_parser("launcher-build", help="build the admitted launcher")
    host_build.add_argument("--manifest", default="governance/confinement/native-launcher-build-v1.json")
    host_build.add_argument("--source", default="native/ranex-worker-launcher/launcher.c")
    host_build.add_argument("--output", default=".local/ranex/build/strict-local-v1/ranex-worker-launcher")
    host_build.add_argument("--result-dir", default=".local/ranex/host-results")
    host_build.set_defaults(func=lambda args: import_module("ranex.cli.host_workflow").main(args))
    host_install = host.add_parser("launcher-install", help="install the admitted launcher")
    host_install.add_argument("--manifest", default="governance/confinement/native-launcher-build-v1.json")
    host_install.add_argument("--artifact", default=".local/ranex/build/strict-local-v1/ranex-worker-launcher")
    host_install.add_argument("--destination", default=".local/ranex/libexec/strict-local-v1/ranex-worker-launcher")
    host_install.add_argument("--result-dir", default=".local/ranex/host-results")
    host_install.set_defaults(func=lambda args: import_module("ranex.cli.host_workflow").main(args))
    host_probe = host.add_parser("host-probe", help="collect admitted host facts")
    host_probe.add_argument("--result-dir", default=".local/ranex/host-results")
    host_probe.set_defaults(func=lambda args: import_module("ranex.cli.host_workflow").main(args))
    host_qualify = host.add_parser("qualify", help="qualify the current host")
    host_qualify.add_argument("--profile", default="governance/confinement/strict-local-host-v1.json")
    host_qualify.add_argument("--artifact", default=".local/ranex/libexec/strict-local-v1/ranex-worker-launcher")
    host_qualify.add_argument("--manifest", default="governance/confinement/native-launcher-build-v1.json")
    host_qualify.add_argument("--report", default=".local/ranex/qualification/strict-local-v1.json")
    host_qualify.add_argument("--result-dir", default=".local/ranex/host-results")
    host_qualify.set_defaults(func=lambda args: import_module("ranex.cli.host_workflow").main(args))
    host_identity = host.add_parser("launcher-identity", help="print launcher and manifest identity")
    host_identity.add_argument("--artifact", default=".local/ranex/libexec/strict-local-v1/ranex-worker-launcher")
    host_identity.add_argument("--manifest", default="governance/confinement/native-launcher-build-v1.json")
    host_identity.set_defaults(func=lambda args: import_module("ranex.cli.host_workflow").main(args))
    strict_local = host.add_parser("strict-local", help="run the strict-local host workflow")
    strict_local.add_argument("--version", choices=("v1", "v2", "v3"), required=True)
    strict_local.add_argument("--runtime-input-path")
    strict_local.add_argument("--toolchain-root")
    strict_local.add_argument("--runtime-closure-root")
    strict_local.add_argument("--result-dir", default=".local/ranex/host-results")
    strict_local.add_argument("--skip-build", action="store_true")
    strict_local.add_argument("--claim", required=True)
    strict_local.add_argument("--producer", required=True)
    strict_local.add_argument("--repository", default=None)
    strict_local.add_argument("--evidence", default=None)
    strict_local.add_argument("--producers", default=None)
    strict_local.add_argument("--gate", default=None)
    strict_local.add_argument("--gate-catalog", default=None)
    strict_local.add_argument("--suite-manifest", default=None)
    strict_local.add_argument("--store", default=None)
    strict_local.add_argument("command", nargs=argparse.REMAINDER)
    strict_local.set_defaults(func=lambda args: import_module("ranex.cli.host_workflow").main(args))
