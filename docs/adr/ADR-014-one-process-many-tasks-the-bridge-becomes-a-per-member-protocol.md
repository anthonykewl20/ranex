...
Parent decisions: ADR-008 (the bridge), ADR-009 (materialisation), ADR-010 (delegation). Related: SLICE-010 (kernel merges); ADR-013 (prototype gate — the owner records the manager as outside it on 2026-08-06). The milestone and slice issues are tracked in this repository's GitHub milestone under the owner's account; the harness fork is upstream code and is never written to. The harness-side mechanics (worktree creation, durable schema, leases, verification, orchestrator, UI) are separate harness decisions, not this ADR.