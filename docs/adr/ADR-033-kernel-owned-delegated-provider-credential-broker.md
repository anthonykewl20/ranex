...
- 9. Any locally valid distinct request reserves before network I/O; upstream timeout, HTTP failure, DNS/connect/TLS failure, pre-first-byte disconnect, malformed SSE, EOF, client cancellation, oversize, or redirect consumes the slot and maps to exactly one non-overlapping named outcome; no retry. A replay is rejected before reservation and consumes zero.
