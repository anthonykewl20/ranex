...
for entry_point in entry_points(group='example.plugins'):
    entry_point.load()
```
...
!!! important
    ...
    for use with a single project in CI or Docker images.
...
example, to build `deepspeed` with `torch` as an additional build dependency:
...
This will ensure that `deepspeed` is built with the same version of `torch` that is or will be
...
!!! note
    ...
    The `FLASH_ATTENTION_SKIP_CUDA_BUILD` environment variable enables `flash-attn` to be resolved
    from a pre-built wheel, rather than attempting to build it from source, which
    ...
    `FLASH_ATTENTION_SKIP_CUDA_BUILD` variable, as setting `FLASH_ATTENTION_SKIP_CUDA_BUILD` to
    `TRUE` can lead to an incompatible install if no compatible pre-built wheel is available
...
example, in the case of `flash-attn`, upgrading the version of `torch` used in the project would
...
supported `match-runtime = true` out of the box. Unlike `axolotl`, though, older versions of
`flash-attn` did not vary their dependencies based on dynamic properties of the build environment.
As such, users could instead provide the `flash-attn` metadata upfront via the
...
distribution (like PyTorch), as such packages can _only_ be installed on environments covered by
the set of pre-built binary distributions (wheels) published by that package.
