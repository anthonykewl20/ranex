...
- **TUF** treats root metadata as an anchor that must be present and valid, and
  fails closed rather than proceeding without it: "If the trusted root metadata
  file has expired, abort the update cycle, report the potential freeze attack."
  Its known weakness is stated by the spec itself and is exactly our residual
  risk — TUF cannot bootstrap its own anchor: "We assume that a good, trusted
  copy of this file was shipped… using an out-of-band process", and a non-goal
  is "providing a means to bootstrap security":
  <https://theupdateframework.github.io/specification/latest/>
- **SLSA v1.0** puts the control outside the tenant: at Build L2 provenance
  "MUST be generated by the control plane… and not by a tenant". Known weakness:
  SLSA is explicit that it says nothing about whether the source itself is
  sound, which is why a committed catalog can still declare a weak gate:
  <https://slsa.dev/spec/v1.0/requirements>
- **Kubernetes admission webhooks** default to fail-closed — `failurePolicy`
  "defines how unrecognized errors from the admission endpoint are handled…
  Defaults to Fail". Its known weakness is the cost we inherit: a fail-closed
  admission path can wedge the system it guards, which is why the control test
  below exists at all:
  <https://kubernetes.io/docs/reference/kubernetes-api/extend-resources/validating-webhook-configuration-v1/>
- **git** documents why a gitignored trust root is worse than an untracked one:
  a `gitignore` file "specifies intentionally untracked files that Git should
  ignore", and "Files already tracked by Git are not affected". Ignored files
  never reach `git status`, so the operator's one habitual signal says nothing:
  <https://git-scm.com/docs/gitignore>
- **Saltzer & Schroeder (1975)**, fail-safe defaults — "base access decisions on
  permission rather than exclusion". The early return was exclusion by default:
